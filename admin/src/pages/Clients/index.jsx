...
const PER_PAGE = 20
const API_BASE = import.meta.env.VITE_API_URL?.replace('/admin', '') || 'http://localhost:3001'
          ...
          <table className={s.table}>
            ...
            <tbody>
              {clients.map(c => {
                ...
                const initials = `${(c.last_name || '')[0] || ''}${(c.first_name || '')[0] || ''}`.toUpperCase()
                return (
                  <tr key={c.id}>
                    <td style={{ width: 50 }}>
                      {photo ? (
                        <img src={photo} alt="" style={{ width: 36, height: 36, borderRadius: '50%', objectFit: 'cover' }} />
                      ) : (
                        <div style={{
                    ...
                    <td style={{ fontWeight: 600 }}>{c.last_name} {c.first_name}</td>
                    <td>{c.phone || '—'}</td>
                    <td style={{ fontSize: 13, color: 'var(--gray-500)' }}>{c.email || '—'}</td>
                    <td style={{ fontSize: 13, color: 'var(--gray-400)' }}>{c.code || '—'}</td>
